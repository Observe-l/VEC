...
from sqliteUtil.test_sqlite import insert_data

class A2EnvSQL:
    def __init__(self,b):
        deleteAll()
        insert_data()
        self.baseStations = selectAll()
        self.b = b      #number of base station
        self.u = 0.1
        self.G_total = np.array([self.baseStations[i].global_computing_resource for i in range(len(self.baseStations))])
        self.Gb_bar = np.array([self.baseStations[i].reversed_computing_resource for i in range(len(self.baseStations))])
        ...
        self.compute_efficiency =  np.array([self.baseStations[i].computing_efficiency for i in range(len(self.baseStations))])
        self.omega1 = 0.2  #range from[0,1]
        self.completion_ratio = np.array([self.baseStations[i].completion_ratio for i in range(len(self.baseStations))])
        self.total_received_task = np.array([self.baseStations[i].total_received_task for i in range(len(self.baseStations))])
        self.omega2 = 0.2
        # self.reliability = self.get_reliability() #state3 reliability
        self.normalized_utility = np.zeros(b)
        self.t_delay = np.zeros(b)
        self.reliability = np.array([self.baseStations[i].total_received_task for i in range(len(self.baseStations))])
    ...
    def update_compute_efficiency(self,Ib):
        ...
        update(Ib,self.baseStations[Ib])
        return
