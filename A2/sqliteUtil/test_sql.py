from sqliteUtil.BSSQLUtil import *

import numpy as np

def insert_data():
    baseStations = [BaseStation() for i in range(2)]
    for i in range(len(baseStations)):
        ...
        baseStations[i].completion_ratio = 0   #0-1  from zequn
        baseStations[i].computing_efficiency = 0  #?
        baseStations[i].total_received_task = 0  #always 0 at beginning
        baseStations[i].reliability = 0 #0-1
