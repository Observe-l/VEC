...
class SimpleWalletTransactionHandler(TransactionHandler):
    ...
    def _make_deposit(self, context, amount, from_key):
        ...
        if current_entry == []:
            LOGGER.info('No previous deposits, creating new deposit {} '
                .format(from_key))
            new_computing_resource = amount
	    
        else:
            computing_resource = current_entry[0].data
            new_computing_resource = amount
    ...
    def _make_transfer(self, context, transfer_amount, to_key, from_key):
        ...
        if transfer_amount <= 0:
            raise InvalidTrd65e72ffd0e0ansaction("The amount cannot be <= 0")
...
def main():
    ...
    try:
        ...
        processor = TransactionProcessor(url='tcp://validator-4:4008')
